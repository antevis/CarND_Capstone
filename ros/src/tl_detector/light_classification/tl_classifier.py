...
#
#
#     def bbox_contents(self, image_np, bbox):
#
#         im_height, im_width = image_np.shape[0], image_np.shape[1]
#
#         y1 = int(im_height * bbox[0])
...
#         return image_np[y1:y2, x1:x2, :]
#
#     def crop(self, image_np, t=.2, b=.3, l=.4, r=.6):
#         top = int(image_np.shape[0] * t)
#         bottom = int(image_np.shape[0] * b)
#         left = int(image_np.shape[1] * l)
#         right = int(image_np.shape[1] * r)
#
...
#                 np_classes = np.squeeze(classes).astype(np.int32)
#
#                 for bbox, score, np_class in zip(np_bboxes, np_scores, np_classes):
#                     if np_class == 10:
#
#                         tl_image_np = self.bbox_contents(image_np=image, bbox=bbox)
#
#                         roi = self.crop(image_np=tl_image_np)
#
#                         # average red channel intensity
